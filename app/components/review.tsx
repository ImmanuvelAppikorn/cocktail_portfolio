    ...
    <div className="px-4 pt-16 space-y-4 relative">
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-3">
          <Image src="/button-image/Icons.svg" alt="Back Icon" height={20} width={20} />
        ...
        <button className="bg-black px-3 py-1.5 rounded-md flex items-center text-white gap-2 hover:bg-gray-800 transition">
          <Image src="/button-image/coffee.svg" alt="Review Icon" height={20} width={20} />
      ...
      <div className="bg-[#F8F8F8] w-full rounded-xl flex items-center px-3 py-2 justify-between">
        <Image src="/button-image/forward.svg" alt="Forward Icon" height={24} width={24} />
        <p className="text-xl font-extrabold">4.0</p>
        <div className="flex -space-x-4">
          {["Ellipse2.svg","Ellipse3.svg","Ellipse4.svg","Ellipse5.svg","Group1.svg"].map((img,i)=>(
            <Image
              key={i}
      ...
      <div className="bg-[#F8F8F8] rounded-lg p-2 flex justify-between items-start">
          ...
          {[5,4,3,2,1].map((num)=>(
            <div key={num} className="flex items-center gap-2">
              ...
              <Image
                ...
                width={5}
              />
              <div className="h-[6px] rounded bg-[#7E8FB3]"
                   style={{width: num===5?"151px":num===4?"106px":num===3?"60px":num===2?"19px":"6px"}} />
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-col items-end space-y-2">
          ...
          <div className="flex gap-1">
            {[1,2,3,4].map((i)=>(
              <Image key={i} src="/start-rating-icons/start-rating-bottle-full.svg" alt="Full Bottle" height={16} width={5}/>
            ))}
            <Image src="/start-rating-icons/start-rating-bottle-empty.svg" alt="Empty Bottle" height={16} width={5}/>
          </div>
          ...
          {reviews.map((r)=>(
            <div key={r.id} className="flex flex-col gap-1">
              <div className="flex items-center gap-2">
                <Image src={r.avatar} alt={r.user} width={30} height={30} className="rounded-full"/>
                <div>
                  ...
                  <div className="flex items-center gap-1.5">
                    <span className="flex gap-0.5">
                      {[...Array(5)].map((_,i)=>(
                        <Image
                          key={i}
                          src={i<r.rating ? "/start-rating-icons/start-rating-bottle-full.svg" : "/start-rating-icons/start-rating-bottle-empty.svg"}
                          alt="Rating Bottle"
                          width={5} height={14}
                        />
                      ))}
