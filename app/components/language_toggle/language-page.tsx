...
const LanguageToggle = ({ onClose }: { onClose: () => void }) => {
  ...
  return (
    <div className="w-full max-w-md mx-auto rounded-3xl bg-white shadow-xl relative px-4 pb-6">
      {/* Close Button */}
      <button
        onClick={onClose}
      ...
      <div className="grid grid-cols-2 gap-3 px-2 sm:px-4">
        {languageItems.map((item, i) => (
          <div key={i} className="flex flex-col items-center">
            <Image src={item.image} alt={item.text} height={90} width={90} />
            <p className="text-[18px] font-bold mt-2">{item.text}</p>
          </div>
        ))}
      </div>

      {/* Button */}
      <div className="bg-[#EC2960] w-full sm:w-[85%] h-[45px] flex justify-center mx-auto rounded-[8px] mt-6 mb-6">
        <button className="text-white text-[16px] font-bold leading-[23px]">
          Click to Continue
        </button>
      </div>
    </div>
